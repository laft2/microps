  ...
  case TCP_PCB_STATE_SYN_SENT:
    ...
    if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
      ...
      if (pcb->snd.una > pcb->iss) {
        ...
        /* NOTE: not specified in the RFC793, but send window initialization
         * required */
        pcb->snd.wnd = seg->wnd;
        pcb->snd.wl1 = seg->seq;
        pcb->snd.wl2 = seg->ack;
        sched_wakeup(&pcb->ctx);
        /* ignore: continue processing at the sixth step below where the URG bit
         * is checked */
        return;
      } else {
        pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
        tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
        /* ignore: If there are other controls or text in the segment, queue
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src,
  ...
  if (src == IP_ADDR_BROADCAST || dst == IP_ADDR_BROADCAST) {
    errorf("broadcast address is not supported in TCP");
    return;
  }
  ...
  mutex_lock(&mutex);
  tcp_segment_arrives(&seg, hdr->flg, (uint8_t *)hdr + hlen, len - hlen, &local,
                      &foreign);
  mutex_unlock(&mutex);
  return;
}
...
int tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign,
                    int active) {
  struct tcp_pcb *pcb;
  char ep1[IP_ADDR_STR_LEN];
  char ep2[IP_ADDR_STR_LEN];
  int state, id;
...
int tcp_close(int id) {
  ...
  case TCP_PCB_STATE_CLOSE_WAIT:
    tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_FIN, NULL, 0);
    pcb->snd.nxt++;
    pcb->state = TCP_PCB_STATE_LAST_ACK; /* RFC793 says "enter CLOSING state",
