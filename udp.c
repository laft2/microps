...
static void udp_input(const uint8_t *data, size_t len, ip_addr_t src,
  ...
  debugf("%s:%d => %s:%d, len=%zu (payload=%zu)",
         ...
         ip_addr_ntop(dst, addr2, sizeof(addr2)), len, len - sizeof(*hdr));
  udp_dump(data, len);
}
...
int udp_open(void) { return 0; }

int udp_close(int id) { return 0; }

int udp_bind(int index, struct ip_endpoint *local) { return 0; }