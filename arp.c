...
static int arp_request(struct net_iface *iface, ip_addr_t tpa) {
  ...
  return net_device_output(iface->dev, NET_PROTOCOL_TYPE_ARP,
...
static int arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa,
  ...
  if (iface->dev->type != NET_DEVICE_TYPE_ETHERNET) {
    errorf("net device type must be ethernet");
    return -1;
  }
  memcpy(&reply.sha, iface->dev->addr, sizeof(reply.sha));
  memcpy(&reply.tha, tha, sizeof(reply.sha));
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  uint16_t hrd = ntoh16(msg->hdr.hrd);
  if (hrd != ARP_HRD_ETHER || msg->hdr.hln != ETHER_ADDR_LEN) {
    errorf("non-ethernet address is rejected for hw address");
    return;
  }
  uint16_t pro = ntoh16(msg->hdr.pro);
  if (pro != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
    errorf("non-IP address is rejected for protocol address");
    return;
  }
